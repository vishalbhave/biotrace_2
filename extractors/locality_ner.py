...
Pipeline:
  ...
  Stage 3  SEGREGATE    — LocalitySplitter (from extractors.gnv.py)
...
class LocalityNER:
    ...
    def extract_localities(
        ...
        # NEW: Filter out entities that match author names
        filtered_entities = []
        for raw, start, end in raw_entities:
            parts = [p.lower() for p in raw.split()]
            if raw.lower() not in author_blacklist and not any(p in author_blacklist for p in parts):
                filtered_entities.append((raw, start, end))
                
        # Segregate comma-lists
        segregated: list[tuple[str,int,int]] = []
        for raw, start, end in filtered_entities[:max_entities]:
    ...
    def enrich_occurrences(
        ...
        """
        localities = self.extract_localities(text, author_blacklist=author_blacklist)
