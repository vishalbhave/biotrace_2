...
Usage:
    from .ner import TaxonNER, TaxonCandidate
    ner = TaxonNER()
...
def _call_gna_batch(names: list[str], cache: Optional[_GNACache] = None) -> dict[str, dict]:
    ...
    try:
        # FIX: Use POST and pass nameStrings in the JSON body
        payload = {
            "nameStrings": uncached,
            "preferredSources": GNA_SOURCES, # Ensure this is an array of IDs
        ...
        # The response structure remains similar
        for item in r.json().get("names", []):
            submitted = item.get("name", "")
            best      = item.get("bestResult") or {}
            parsed    = _parse_gna_best(best)
        ...
        # Throttling is good practice for bulk calls
        time.sleep(0.2)
