...
───────────────────────────────────────────────────────────
    from .agentic_chunker import AgenticExtractionPipeline
...
try:
    from utils.schema import OccurrenceRecord, RelationTriple
    _SCHEMA_OK = True
...
try:
    from .scientific_chunker import ScientificPaperChunker as _FallbackChunker
    _FALLBACK_OK = True
...
def _record_key(rec) -> tuple:
    ...
    if hasattr(rec, 'recorded_name'):
        # OccurrenceRecord (pydantic model from utils.schema v5.4)
...
class AgenticExtractionPipeline:
    ...
    def _run_raw_extraction(
        ...
        try:
            from utils.schema import parse_llm_response as _parse_llm
        except ImportError:
            _parse_llm = None
...
def build_agentic_pipeline(
    ...
    Usage in biotrace_v53.py (replaces ScientificPaperChunker block):
        ...
        from .agentic_chunker import build_agentic_pipeline
