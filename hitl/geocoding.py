...
Usage (unchanged from v5.5):
    from .geocoding import render_hitl_geocoding_tab
    with tabs[N]:
...
def _kg_update_locality_coords(kg_db_path: str, locality: str,
    ...
    try:
        from knowledge_graph.knowledge_graph import BioTraceKnowledgeGraph, _node_id, _now
...
def _kg_delete_species(kg_db_path: str, valid_name: str) -> bool:
    try:
        from knowledge_graph.knowledge_graph import BioTraceKnowledgeGraph, _node_id
        kg = BioTraceKnowledgeGraph(kg_db_path)
...
def _re_verify_occurrence(occurrence: dict) -> dict:
    try:
        from utils.unified_verifier import BioTraceUnifiedVerifier
