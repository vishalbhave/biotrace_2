...
Usage:
  from .gbif_verifier import gbif_verify_batch, render_approval_table
"""
...
def render_approval_table(occurrences: list[dict]) -> list[dict] | None:
    ...
    Wire into biotrace_v5.py after GBIF verification:
        ...
        from .gbif_verifier import render_approval_table
    ...
    # 1. Find which rows in the editor were checked as approved
    approved_indices = edited.index[edited["approve"]].tolist()

    for idx in approved_indices:
        # 2. Get the original index from our complete 'rows' list
        orig_idx = rows[idx]["_orig_idx"]
        
        # 3. Grab the full dictionary from the original data
        occ = occurrences[orig_idx]
        
        # 4. Grab the user's edited text for this specific row
        edited_row = edited.iloc[idx]
        
        # 5. Map the edited table columns back to the database keys
        occ["recordedName"]     = edited_row["recordedName"]
        occ["gbifName"]         = edited_row["gbifName"]
        occ["wormsID"]          = edited_row["wormsID"]
        occ["family_"]          = edited_row["family"]     # Map 'family' -> 'family_'
        occ["verbatimLocality"] = edited_row["locality"]   # Map 'locality' -> 'verbatimLocality'
        occ["occurrenceType"]   = edited_row["occType"]    # Map 'occType' -> 'occurrenceType'
        
        # 6. Add the fully updated dictionary to our final approved list
        approved_occurrences.append(occ)
