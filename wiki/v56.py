...
with:
    ...
    from .v56 import BioTraceWikiV56 as BioTraceWikiUnified, inject_css_streamlit
...
def _to_markdown_v56(art: dict) -> str:
    ...
    if occ_pts:
        ...
        for pt in occ_pts[:50]:
            ...
            dep   = pt.get("depth_m")
            dep_s = f" · {dep} m depth" if dep else ""
            rec_s = (f" [recorded as *{rec_n}*]"
                     if rec_n and rec_n != pt.get("validName", "") else "")

            # Year: prefer samplingEvent.year, fall back to samplingEvent.date[:4]
            ...
            if isinstance(samp, str):
                try:
                    ...
                    samp = {}
            yr_raw = samp.get("year") or samp.get("eventDate") or samp.get("date")
            yr_s = f" · {str(yr_raw)[:4]}" if yr_raw else ""

            lines.append(f"- **{loc}** ({ot}{dep_s}{yr_s}){rec_s} — _{src}_")
        lines.append("")
