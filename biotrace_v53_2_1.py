...
try:
    from biotrace_unified_wiki import GeocodingCascade
...
try:
    ...
    from biotrace_unified_wiki import BioTraceWikiUnified
    _WIKI_AVAILABLE = True
...
try:
    ...
    from biotrace_hitl_ml_framework import initialize_hitl_pipeline, enrich_occurrence_record
...
try:
    ...
    from biotrace_hitl_streamlit_integration import render_hitl_verification_tab
...
from biotrace_unified_wiki import LocalityIdentifier as _LocalityIdentifier
...
tabs = st.tabs([
    ...
    "📥 Export",
    "Scientific Wiki",
    "🤖 ML Verification" # Add this 13th tab
])
...
with tabs[7]:
    ...
    gq = st.text_area(
        ...
        placeholder=(
            ...
            "e.g. List Acacia species from Kenya, or nudibranchs from Gulf of Mannar"
        ),
        height=100,
    )
    ...
    if st.button("🔎 Run GraphRAG Query", type="primary") and gq.strip():
        with st.spinner("Building context + querying LLM…"):
            ...
            if not full_context.strip():
                ...
                full_prompt = (
                    "You are a biodiversity expert covering all taxa — marine, "
                    "terrestrial, freshwater, and botanical. Use the structured context "
                    "below (from a knowledge graph, memory bank, and wiki) to answer "
                    "the question accurately. Cite species with their scientific names\n\n"
                    "and include authoritative taxonomic IDs where available "
                    "(WoRMS for marine, GBIF/ITIS for terrestrial, POWO for plants).\n\n"
                    f"{full_context}\n\n"
                    f"QUESTION: {gq}\n\n"
                    "Answer with taxonomic precision appropriate to the taxon group."
                )
                response = call_llm(
