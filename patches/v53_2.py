...
from .v56_integration import install_v56_patches
install_v56_patches(meta_db_path=META_DB_PATH,
                    kg_db_path=KG_DB_PATH, 
                    wiki_root=WIKI_ROOT)


from .patch57_update import install_v57_patches
install_v57_patches(
...
class BioCentricExtractor:
    ...
    def _build_species_prompt(self, species: str, context_window: str, loc_context: list[str] = None) -> str:
        ...
        • Exact locations (locality names, site codes, coordinates) {loc_str}
...
def process_chunk(
    ...
    if gna_pipeline:
        ...
        if sp_windows:
            ...
            biocentered = BioCentricExtractor(context_window=800, log_cb=log_cb)
            biocentric_recs = biocentered.extract_per_species(
                text=augmented_text, species_list=unique_species[:30],
                provider=provider, model_sel=model_sel, api_key=api_key,
                ollama_base_url=ollama_base_url,
            )
            for rec in biocentric_recs:
                _cast_vote(rec, "biocentric")
    ...
    try:
        ...
        # Fallback to standard JSON extraction if Schema validator fails silently
        if not llm_data:
            clean_json = re.sub(r"<think>.*?</think>", "", raw, flags=re.DOTALL).strip()
            # fence = re.search(r"```json)?\s*(\[.*?\])\s*```", clean_json, re.DOTALL)
            
            fence = re.search(r"```(?:json)?\s*(\[.*?\])\s*```", clean_json, re.DOTALL)
            if fence: clean_json = fence.group(1).strip()
            try:
                llm_data = json.loads(clean_json) if clean_json.startswith("[") else []
            except json.JSONDecodeError:
                llm_data = []
        ...
        if llm_data:
            ...
            log_cb(f"    [StandardLLM] Failed to extract valid JSON (Preview: {raw[:40]}...)", "warn")
...
def ingest_into_v5_systems(
    ...
    if wiki:
        try:
            # Normalize fields before passing to Unified Wiki
            for o in occurrences:
                o["locality"] = o.get("verbatimLocality", "Unknown")
                o["occurrence_type"] = o.get("occurrenceType", "Unknown")
                
                samp = o.get("Sampling Event") or o.get("samplingEvent", {})
...
def render_scientific_wiki_tab():
    ...
    if view_type == 'markdown':
        ...
        # 2. Add the Interactive Folium Map
        st.divider()
        st.subheader("🗺️ Interactive Occurrence Map")
        
        # Fetch occurrences for this specific species from the DB
        df_existing = db_load_all()
        if not df_existing.empty:
            sp_occs = df_existing[df_existing["validName"] == selected_species]
            with_coords = sp_occs.dropna(subset=["decimalLatitude", "decimalLongitude"])
            ...
            if not with_coords.empty:
                try:
                    ...
                    avg_lat = with_coords["decimalLatitude"].mean()
                    avg_lon = with_coords["decimalLongitude"].mean()
                    ...
                    for _, row in with_coords.iterrows():
                        loc = row.get("verbatimLocality", "Unknown")
                        o_type = row.get("occurrenceType", "Uncertain")
                        
                        # Use different colors based on Primary vs Secondary
                        color = "green" if str(o_type).lower() == "primary" else "blue"
                        ...
                        folium.CircleMarker(
                            ...
                            fill_color=color,
                            fill_opacity=0.7,
                            tooltip=f"<b>{selected_species}</b><br>{loc}",
                            popup=f"<b>Locality:</b> {loc}<br><b>Type:</b> {o_type}<br><b>Citation:</b> {row.get('sourceCitation', 'N/A')[:50]}..."
                        ).add_to(m)
                        
                    st_folium(m, width=800, height=450, returned_objects=[])
                except ImportError:
                    st.warning("⚠️ `folium` or `streamlit-folium` not installed. Falling back to simple map.")
                    map_df = with_coords.rename(columns={"decimalLatitude":"lat", "decimalLongitude":"lon"})[["lat", "lon"]]
                    st.map(map_df)
            else:
                st.info("📍 No coordinate data available for this species to display on the map.")
            ...
            if st.button("🗺️ Generate Interactive Graph (PyVis)"):
                with st.spinner("Rendering…"):
                    ...
                    if out and os.path.exists(out):
                        ...
                        st.iframe(src=html_content, width='stretch', height=600, scrolling=True)
...
with tabs[6]:
    ...
    if not wiki:
        ...
        with col_w1:
            st.subheader("📖 BioTrace Unified Wiki")
        with col_w2:
            # 1. THE REBUILD BUTTON
            if st.button("🔄 Rebuild from Database", use_container_width=True):
                with st.spinner("Fetching and normalizing database records..."):
                    df_all = db_load_all()
                    if not df_all.empty:
                        db_occs = df_all.to_dict("records")
                        
                        # 2. THE DATA NORMALIZER (Frees Temporal/Depth data & fixes mapping)
                        for o in db_occs:
                            # Map flat fields
                            o["locality"] = o.get("verbatimLocality") or o.get("verbatimlocality", "Unknown")
                            o["occurrence_type"] = o.get("occurrenceType") or o.get("occurrencetype", "Unknown")
                            o["latitude"] = o.get("decimalLatitude") or o.get("decimallatitude")
                            o["longitude"] = o.get("decimalLongitude") or o.get("decimallongitude")
                            
                            # Parse the JSON string to free Temporal and Depth data
                            samp = o.get("samplingEvent") or o.get("samplingevent", "{}")
                            if isinstance(samp, str):
                                try:
                        ...
                        # Send normalized data to the Wiki
                        ...
                        st.success("✅ Wiki successfully rebuilt from existing database chunks!")
                    else:
                ...
                if sci_wiki_gen is None:
                    ...
                    for i, sp_name in enumerate(ungenerated):
                        ...
                        class _MockOcc:
                            def __init__(self, o):
                                self.canonical_name      = sp_name
                                # Map both standard and verbatim names
                                self.recorded_name       = o.get("recordedName", sp_name)
                                # The wiki engine likely expects verbatim_locality or locality
                                self.locality            = o.get("verbatimLocality", "Unknown")
                                self.verbatim_locality   = o.get("verbatimLocality", "Unknown")
                                # Map the occurrence type (Primary/Secondary)
                                self.occurrence_type     = o.get("occurrenceType", "Unknown")
                                self.country             = ""
                                self.state_province      = ""
                                self.event_date          = ""
                                # Handle depth if you extract it in the sampling event
                                self.depth_m             = o.get("depth", "") 
                                self.latitude            = o.get("decimalLatitude")
                                self.longitude           = o.get("decimalLongitude")
