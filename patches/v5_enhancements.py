...
TaxonNER = None
try:
    from extractors.ner import TaxonNER, regex_scan, trigram_score, extract_taxa
    _NER_AVAILABLE = True
...
try:
    from extractors.gnv import GNVEnrichedVerifier, LocalitySplitter, dedup_occurrences
    _GNV_AVAILABLE = True
except ImportError:
    pass
    ...
    if run_btn and text_input.strip():
        ...
        if candidates:
            ...
            if os.path.exists(mb_path):
                from extractors.ner import COPIOUSFilter
                cop = COPIOUSFilter()
